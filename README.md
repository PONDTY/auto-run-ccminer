...
```
apt-get update -y && apt-get install git -y && git clone https://github.com/PONDTY/auto-run-ccminer && cd auto-run-ccminer && chmod +x setup.sh && sh setup.sh
```
...
```
```
git clone https://github.com/PONDTY/auto-run-ccminer
```
```
cd auto-run-ccminer
```
